...
**Fuzz-target registry**: The package-owned catalogue (`.github/fuzz-targets.json`) of fuzz targets that every fuzz harness consumes — the local Task runner, the nightly and autoscaled CI campaigns, and the OSS-Fuzz-compatible staging build. Each entry names the target function, its package, and its OSS-Fuzz-compatible binary name. A root-package drift test makes the registry authoritative: the defined fuzz functions, the registry, and the OSS-Fuzz-compatible build lines must agree or `go test` fails.
_Avoid_: fuzz list, target matrix, build manifest.

**Exchange fixture**: The test-surface module that owns four-step exchange plumbing for white-box tests: it runs Start and Respond eagerly with the package's deterministic test randomness, exposes every intermediate handle, provides happy-path finish helpers that fail the test on error, and owns the **Secret snapshot** adapters. Failure-path tests drive the public API directly on the fixture's handles; Start/Respond-failure tests use the raw deterministic seams. Fuzz seeds and benchmarks share its input builder without adopting its staged methods.
_Avoid_: test harness, exchange helper, setup function.

**Secret snapshot**: The Exchange fixture's per-role adapter for hygiene assertions: taken while a **CPace core**'s or **Session**'s persistent secrets are live, it captures aliases to their backing arrays, and after a terminal operation asserts the owner dropped its references and the captured arrays were zeroed. It is the only test-surface module that learns a core's or session's field layout through public state; hygiene tests snapshot before Finish or Close and assert after, never reaching into a live exchange's core themselves. Unit tests that construct a core directly sit outside its scope.
_Avoid_: zeroization check, cleanup assertion, struct peek.
