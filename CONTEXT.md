...
**Secret snapshot**: The Exchange fixture's per-role adapter for hygiene assertions: taken while a **CPace core**'s persistent secrets are live, it captures aliases to their backing arrays, and after a terminal operation asserts the core dropped its references and the captured arrays were zeroed. It is the only test-surface module that learns a core's field layout through the public single-use state; hygiene tests snapshot before Finish or Close and assert after, never reaching into a live exchange's core themselves. Unit tests that construct a core directly sit outside its scope.
