...
The historical exact-candidate packet at `f7efa6a963a954952b1ecad3f46530f13799fe89`, captured under Go 1.26.4 in `docs/evidence/f7efa6a-20260619/`, remains the strongest completed bundle for paired long fuzzing, tag-ruleset state, GitHub status, and Scorecard. Do not treat those lanes as current for `v0.1.3`; its Capslock, security/spec, and vector artifacts are superseded by the exact-candidate protocol bundle for those specific lanes.
...
Before the `v0.1.3` candidate gate, refresh paired long fuzzing, tag-ruleset state, GitHub alert state, Scorecard, and release-validation evidence against the same exact candidate commit. Keep the completed dependency/SAST and protocol/capability/vector lanes pinned to their raw checked bundles unless a refresh trigger selects a new candidate.
