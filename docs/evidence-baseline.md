...
The frozen `v0.1.3` candidate source baseline is `1f19e278112fa037890848ed6c086addeffdca4e`, selected from a clean worktree under Go 1.26.5 after the PR #227 disposition and toolchain update. The dependency, vulnerability, and SAST/gosec lane is now fresh for that exact candidate; naming the SHA alone does not make the other lanes fresh.

The current dependency, vulnerability, and SAST/gosec evidence is the clean-worktree Go 1.26.5 capture in `docs/evidence/1f19e27-20260731/`. Module integrity passed, `govulncheck@v1.3.0` reported no vulnerabilities, and the test-inclusive pinned `gosec@v2.26.1` scan reported zero issues. This completed lane is prerelease evidence, not a production-readiness claim.

The strongest completed multi-lane release-evidence packet remains the historical exact-candidate packet at `f7efa6a963a954952b1ecad3f46530f13799fe89`, captured under Go 1.26.4 in `docs/evidence/f7efa6a-20260619/`. It remains historical for Capslock, paired long fuzzing, security/spec and vector stability, tag-ruleset state, GitHub status, and Scorecard; do not treat those lanes as current for `v0.1.3`.
...
Before the `v0.1.3` candidate gate, run and preserve the Go 1.26.4 to Go 1.26.5 vector comparison, recording whether outputs are bit-identical or explicitly recording that the older toolchain is unavailable. Refresh Capslock, paired long fuzzing, security/spec audit, tag-ruleset state, GitHub alert state, Scorecard, and release-validation evidence against the same exact candidate commit.
