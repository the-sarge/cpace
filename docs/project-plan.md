...
3. Complete the remaining exact-candidate evidence for issue #33. The dependency/govulncheck and test-inclusive gosec lane is current at the frozen candidate; refresh Capslock, paired long fuzz, security/spec audit, vector stability, tag-ruleset, GitHub alert, Scorecard, and release-validation evidence against the same exact candidate.
