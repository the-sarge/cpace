...
| --- | --- | --- |
| Dependency review | `docs/dependency-review.md` records `govulncheck -test -show verbose ./...` and pinned `gosec@v2.26.1` results under Go 1.26.3 for commit `737bc56ffba81e2df5e9caa0df1ff180bfdb594b`. | Repeat on the exact release tag if dependencies, toolchain, or parser/security-relevant code changes. |
| Long fuzz evidence | `docs/fuzz-evidence.md` records all 14 registered targets for `FUZZTIME=1h` on paired local ARM/Intel Go 1.26.3 runs for commit `737bc56ffba81e2df5e9caa0df1ff180bfdb594b`; earlier Go 1.26.2 and older ARM/Intel runs remain historical evidence. | Repeat if parser, protocol, fuzz harness, dependency, or toolchain changes before release. |
...
| Capslock capability analysis | `docs/capslock-report.md` records Capslock `v0.3.2` results under Go 1.26.3 for main commit `737bc56ffba81e2df5e9caa0df1ff180bfdb594b`. | Capslock is experimental review signal, not a release gate. Repeat if dependencies, imports, randomness, HKDF/HMAC usage, or the Go toolchain change. |
