...
| GitHub release controls | `docs/evidence/1f19e27-20260801-github/` preserves checksummed tag-ruleset, branch-protection, required-check, alert, Scorecard, and scheduled-workflow state captured at protected `main` `fa4ea296daae82e920ae6bb410e9bcb7c0061641`, including Nightly Fuzz failure/recovery and autoscaled queue history. | Current point-in-time prerelease signal for the frozen candidate contract, not exact-candidate execution for scheduled runs on older SHAs. The source candidate is an intermediate PR #239 commit without direct check suites; all required contexts passed on its checked documentation-only descendant. Recapture if GitHub state changes or becomes stale before tagging. |
| Assembled candidate packet | `docs/evidence/1f19e27-20260801-candidate/` hash-links every current `v0.1.3` evidence lane, preserves the extracted release notes, and records the full pre-tag gate against a clean documentation-only descendant of the frozen source. | The packet is prerelease self-evidence, not independent review. Signed-tag Release Validation, post-SARIF Code Scanning, release asset verification, and publication tracking remain post-tag work. |
