...
3. Complete the remaining exact-candidate evidence for issue #33. The dependency/govulncheck, test-inclusive gosec, Capslock, security/spec audit, and Go 1.26.4 to Go 1.26.5 vector-stability lanes are current at the frozen candidate; refresh paired long fuzz, tag-ruleset, GitHub alert, Scorecard, and release-validation evidence against that same exact candidate.
