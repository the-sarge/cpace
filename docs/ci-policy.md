...
- `task check:changed` collects branch, staged, unstaged, and untracked changes and runs docs validation or `quick`, plus the existing evidence lanes when relevant. The central classifier adds `release:policy` and `release:lint` for changes to `tools/releasepolicy/**`, its wrapper, the classifier scripts, `Taskfile.yml`, the release or SAST workflow, allowed signers, Syft configuration, required release helpers, or the Go-tool catalogue. Unrelated changes keep the lightweight lane. Keep that trigger set aligned with the release checker inputs when maintaining the checker.
