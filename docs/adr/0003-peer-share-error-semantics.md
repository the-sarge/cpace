...
- **Each exported sentinel is reachable from at least one test** — `TestPeerShareErrorsWrapErrAbort/respond_non-canonical` and `TestPeerShareErrorsWrapErrAbort/respond_identity`. These subtests now retain the coverage originally named `TestPeerShareEncodingRejection` and `TestPeerShareIdentityRejection`, respectively; this reference update follows the fixture consolidation in PR #291 and maintains traceability without changing the accepted decision.
...
5. Add tests:
   ...
   - `TestPeerShareErrorsWrapErrAbort/respond_non-canonical` (originally `TestPeerShareEncodingRejection`) — feed a non-canonical Ristretto255 encoding via wire and assert `ErrPeerShareEncoding`.
   - `TestPeerShareErrorsWrapErrAbort/respond_identity` (originally `TestPeerShareIdentityRejection`) — feed the identity encoding via wire and assert `ErrPeerShareIdentity`.
