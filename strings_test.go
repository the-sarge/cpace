	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !bytes.Equal(tc.got, want) {
				t.Fatalf("encoded string got %x want %x", tc.got, want)
			}
		})
	}
}
...
func TestIRTranscriptDraftVectorFlow(t *testing.T) {
	...
	if !bytes.Equal(tr.bytes(), wantTranscript) {
		t.Fatalf("IR transcript got %x want %x", tr.bytes(), wantTranscript)
	}
	isk := tr.deriveISK(v["sid"], v["K"])
	if !bytes.Equal(isk, v["ISK_IR"]) {
		t.Fatalf("ISK_IR got %x want %x", isk, v["ISK_IR"])
	}
	if got := tr.responderConfirmationTag(isk, v["sid"]); !bytes.Equal(got, tags["tagB"]) {
		t.Fatalf("responder confirmation tag got %x want %x", got, tags["tagB"])
	}
	if got := tr.initiatorConfirmationTag(isk, v["sid"]); !bytes.Equal(got, tags["tagA"]) {
		t.Fatalf("initiator confirmation tag got %x want %x", got, tags["tagA"])
	}
	...
	if got := tr.transcriptID(); !bytes.Equal(got, wantTranscriptID) {
		t.Fatalf("TranscriptID got %x want %x", got, wantTranscriptID)
	}
}
...
func TestIRTranscriptInitiatorAD(t *testing.T) {
	...
	if !bytes.Equal(got, []byte("ada")) {
		t.Fatalf("initiatorAD got %q want %q", got, "ada")
	}
...
func TestWireFormatPrefixByte(t *testing.T) {
	if wireFormatV1 != 0xc1 {
		t.Fatalf("wireFormatV1 got %#x want 0xc1", wireFormatV1)
	}
	if wireSuite != 0x01 {
		t.Fatalf("wireSuite got %#x want 0x01", wireSuite)
	}
	cases := []struct {
		name string
		msg  []byte
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.msg[0] != wireFormatV1 {
				t.Fatalf("format prefix got %#x want %#x", tc.msg[0], wireFormatV1)
			}
			if tc.msg[1] != wireSuite {
				t.Fatalf("suite byte got %#x want %#x", tc.msg[1], wireSuite)
			}
		})
	}
}
