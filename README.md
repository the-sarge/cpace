...
Current work is release readiness, not new policy design. The public API and package-profile choices are frozen for review unless a new finding reopens a decision. ADR-0008 records the public-lifecycle thaw that shipped `Initiator.Close` and `Responder.Close`; ADR-0009 records the Caller input replacement that shipped role-local `Input` in v0.1.3. Those implementation authorizations are complete, and all unrelated public-surface and package-profile choices remain frozen. Before any production-readiness claim, the release bar in `docs/security-assessment.md` must be satisfied.
...
`task quick` and `task check` require the normal Go/tooling prerequisites plus `jq` for fuzz-registry validation; the full gate also runs release-helper smoke tests that validate CycloneDX SBOM JSON. `task lint:golangci` runs the curated `golangci-lint` analyzer set over the root module and is the single owner of `gosec`, `staticcheck`, and `go vet` coverage there; `task lint` and therefore `task check` now run it. `.golangci.yml` is a verbatim copy of the portfolio Go lint standard v1.0.0 and is not edited here; `task lint:config-check` pins it by sha256 and `task lint` (and therefore `task check`) fails on drift. `govulncheck` stays a separate lane because it queries the Go vulnerability database rather than analysing source, and the nested `tools/releasepolicy` and `tools/evidencebaseline` modules keep their own `go vet` plus Staticcheck lanes because they are outside the root `golangci-lint` run.

The fuzz-target registry lives in `.github/fuzz-targets.json` and records each target function, package, and OSS-Fuzz-compatible binary name; `go test ./...` includes a drift check that keeps it in sync with defined fuzz functions and `ossfuzz/build.sh`. GitHub-hosted pull-request CI is intentionally light because it runs untrusted fork code: every PR sets up Go, runs the release policy checker and its module tests, and runs `go vet` and Staticcheck for `tools/releasepolicy`; code changes also run `go test ./...` and the evidence baseline validator, while ordinary Markdown-only PRs skip root-module tests and other non-documentation checks unless evidence-document changes require evidence baseline validation. The required `SAST Gate` runs the curated `golangci-lint` set on every PR and uploads its SARIF to Code Scanning. Scheduled hosted lanes run `govulncheck` and a 5-minute-per-target fuzz regression pass as background signal. Run the full local gate, longer maintainer-machine fuzzing, and the vulnerability scan locally before release-oriented changes. The default fuzz lane keeps the race detector on for smoke runs; use `FUZZ_RACE=0` for longer campaigns after `task check` has already covered race-instrumented tests. See `docs/ci-policy.md` for the hosted and self-hosted runner threat model.
