...
`task quick` and `task check` require the normal Go/tooling prerequisites plus `jq` for fuzz-registry validation; the full gate also runs release-helper smoke tests that validate CycloneDX SBOM JSON. `task lint:golangci` runs a curated advisory analyzer set and is not part of the required local gate.
